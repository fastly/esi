...
use bytes::Bytes;
use esi::{parse_complete, Element, Tag};
...
default content
    ...
    assert!(
        choose_found,
        "Should find Choose tag with when and otherwise"
    );
}
...
fn test_zero_copy_esi_remove_with_tags_inside() {
    ...
    let no_div = !elements
        .iter()
        .any(|element| matches!(element, Element::Html(bytes) if bytes.as_ref().contains(&b'd')));
...
<esi:except>error</esi:except>
    ...
    let has_try = elements
        .iter()
        .any(|element| matches!(element, Element::Esi(Tag::Try { .. })));
